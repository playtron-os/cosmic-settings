    ...
    let bindings = model.bindings.iter().enumerate().fold(
        ...
        |section, (_, (bind_id, shortcut))| {
            ...
            if shortcut.is_saved {
                let delete_button = widget::button::icon(icon::from_svg_bytes(icetron_assets::icons::system::DELETE_BIN_LINE))
...
fn shortcut_item(custom: bool, id: usize, data: &ShortcutModel) -> Element<'_, ShortcutMessage> {
    ...
    let control = widget::row::with_capacity(4)
        ...
        .push_maybe(custom.then(|| {
            widget::button::icon(icon::from_svg_bytes(icetron_assets::icons::system::DELETE_BIN_LINE))
